...
if (isset($_POST['edit'])) {
    $id = $_POST['id'];
    $jadwal = mysqli_query($conn, "select id_jadwal, nama_kelas,jadwal.id_kelas, dosen.nama as nama_dosen, matakuliah.nama as nama_mk, jam_masuk, jam_keluar, hari from jadwal join kelas on jadwal.id_kelas = kelas.id_kelas join dosen on jadwal.nip = dosen.nip join matakuliah on jadwal.id_mk = matakuliah.id where id_jadwal = '$id'");
    $row = mysqli_fetch_array($jadwal, MYSQLI_ASSOC);
}
        ...
        <form action="edit_jadwal.php" method="post">
            ...
            <div class="row">
                ...
                <div class="col mb-3 ">
                    ...
                    <input type="time" class="form-control rounded-3" id="jamMulai" name="jamMulai" value="<?= $row['jam_masuk'] ?>"/>
                </div>
                <div class="col mb-3 ">
                    <label for="jamSelesai">Jam selesai</label>
                    <input type="time" class="form-control rounded-3" id="jamSelesai" name="jamSelesai" value="<?= $row['jam_keluar'] ?>"/>
                </div>
            </div>
